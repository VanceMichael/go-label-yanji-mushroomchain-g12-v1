...
func revokeUserSessions(ctx context.Context, q queryer, userID string, at time.Time) error {
	result, err := q.ExecContext(ctx, `UPDATE sessions SET revoked_at=? WHERE user_id=? AND revoked_at IS NULL`, formatTime(at), userID)
	if err != nil {
		return mapSQLError(err)
	}
	return requireAffected(result, "user sessions")
}

func (s *Store) RevokeUserSessions(ctx context.Context, userID string, at time.Time) error {
	return revokeUserSessions(ctx, s.q(), userID, at)
}
func (s *txStore) RevokeUserSessions(ctx context.Context, userID string, at time.Time) error {
	return revokeUserSessions(ctx, s.qer(), userID, at)
}
