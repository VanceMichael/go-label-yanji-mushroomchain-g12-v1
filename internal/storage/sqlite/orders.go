...
const orderColumns = `id,tenant_id,buyer_name,delivery_region,status,idempotency_key,version,total_cents,requested_at,due_at,created_at,updated_at`
...
func createOrder(ctx context.Context, q queryer, order domain.Order) error {
	...
	_, err := q.ExecContext(ctx, `INSERT INTO orders(`+orderColumns+`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`, order.ID,
		order.TenantID, order.BuyerName, order.DeliveryRegion, order.Status, order.IdempotencyKey, order.Version,
...
func scanOrder(row interface{ Scan(...any) error }) (domain.Order, error) {
	...
	err := row.Scan(&order.ID, &order.TenantID, &order.BuyerName, &order.DeliveryRegion, &order.Status,
		&order.IdempotencyKey, &order.Version, &order.TotalCents, &requested, &due, &created, &updated)
	if err != nil {
...
func createAllocations(ctx context.Context, q queryer, items []domain.InventoryAllocation) error {
	for _, item := range items {
		if err := createAllocation(ctx, q, item); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) CreateAllocations(ctx context.Context, items []domain.InventoryAllocation) error {
	return createAllocations(ctx, s.q(), items)
}
func (s *txStore) CreateAllocations(ctx context.Context, items []domain.InventoryAllocation) error {
	return createAllocations(ctx, s.qer(), items)
}
