...
func recordSettlementApproval(ctx context.Context, q queryer, event domain.OutboxEvent, auditEvent audit.Event) error {
	if err := createOutbox(ctx, q, event); err != nil {
		return err
	}
	return createAudit(ctx, q, auditEvent)
}

func (s *Store) RecordSettlementApproval(ctx context.Context, event domain.OutboxEvent, auditEvent audit.Event) error {
	return s.WithinTx(ctx, func(tx repository.Tx) error {
		return tx.RecordSettlementApproval(ctx, event, auditEvent)
	})
}

func (s *txStore) RecordSettlementApproval(ctx context.Context, event domain.OutboxEvent, auditEvent audit.Event) error {
	return recordSettlementApproval(ctx, s.qer(), event, auditEvent)
}
