...
func allocateBatches(ctx context.Context, q queryer, tenantID string, allocations []repository.BatchAllocation, at time.Time) error {
	for _, allocation := range allocations {
		if err := allocateBatch(ctx, q, tenantID, allocation.BatchID, allocation.Quantity, allocation.Version, at); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) AllocateBatches(ctx context.Context, tenantID string, allocations []repository.BatchAllocation, at time.Time) error {
	return allocateBatches(ctx, s.q(), tenantID, allocations, at)
}
func (s *txStore) AllocateBatches(ctx context.Context, tenantID string, allocations []repository.BatchAllocation, at time.Time) error {
	return allocateBatches(ctx, s.qer(), tenantID, allocations, at)
}
