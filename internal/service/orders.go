...
func FingerprintOrderRequest(input CreateOrderInput) string {
	payload, _ := json.Marshal(struct {
		BuyerName, DeliveryRegion string
		DueAt                     time.Time
		Lines                     []domain.OrderLine
	}{input.BuyerName, input.DeliveryRegion, input.DueAt.UTC(), input.Lines})
...
func (s *OrderService) Create(ctx context.Context, input CreateOrderInput) (domain.Order, error) {
	...
	order := domain.Order{ID: id, TenantID: actor.TenantID, BuyerName: input.BuyerName, DeliveryRegion: input.DeliveryRegion, Status: domain.OrderDraft, IdempotencyKey: input.IdempotencyKey, RequestFingerprint: input.RequestFingerprint, Version: 1, RequestedAt: now, DueAt: input.DueAt.UTC(), CreatedAt: now, UpdatedAt: now}
