...
func (s *OrderService) Allocate(ctx context.Context, input AllocateInput) (domain.Order, error) {
	...
	for _, line := range order.Lines {
		err = s.tx.WithinTx(ctx, func(tx repository.Tx) error {
			current, e := tx.GetOrder(ctx, actor.TenantID, order.ID)
			if e != nil {
				return e
			}
			if current.Version != input.ExpectedVersion || current.Status != domain.OrderConfirmed {
	...
	err = s.tx.WithinTx(ctx, func(tx repository.Tx) error {
		...
		if e = tx.TransitionOrder(ctx, actor.TenantID, order.ID, current.Version, domain.OrderAllocated, now); e != nil {
			return e
		}
