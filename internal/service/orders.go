...
func (s *OrderService) Create(ctx context.Context, input CreateOrderInput) (domain.Order, error) {
	...
	event, err := newAudit(actor, input.RequestID, "order.create", "order", id, audit.OutcomeSucceeded, map[string]any{"lines": len(order.Lines), "total_cents": order.TotalCents}, now)
	if err != nil {
		return domain.Order{}, err
	}
	...
	if err = s.repo.CreateAudit(ctx, event); err != nil {
		return domain.Order{}, err
	}
