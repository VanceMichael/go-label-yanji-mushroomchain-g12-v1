...
func (s *OrderService) Allocate(ctx context.Context, input AllocateInput) (domain.Order, error) {
	...
	err = s.tx.WithinTx(ctx, func(tx repository.Tx) error {
		...
		if order.Status != domain.OrderConfirmed {
			return domain.StateError{Entity: "order", From: string(order.Status), To: string(domain.OrderAllocated)}
		}
		for _, line := range order.Lines {
			candidates, _, e := tx.ListBatches(ctx, repository.BatchFilter{TenantID: actor.TenantID, Species: line.Species, Status: domain.BatchReleased, Page: pagination.Page{Number: 1, Size: 100}})
			if e != nil {
				return e
			}
			remaining := line.Quantity
			for _, batch := range candidates {
				if !batch.ExpiresAt.After(now) || remaining == 0 {
					continue
				}
				quantity := remaining
				...
				if e = tx.AllocateBatch(ctx, actor.TenantID, batch.ID, quantity, batch.Version, now); e != nil {
					return e
				}
				...
				if e = tx.CreateAllocation(ctx, domain.InventoryAllocation{ID: allocationID, TenantID: actor.TenantID, OrderID: order.ID, LineID: line.ID, BatchID: batch.ID, Quantity: quantity, CreatedAt: now}); e != nil {
					return e
				}
				remaining -= quantity
			}
		...
		if e := tx.TransitionOrder(ctx, actor.TenantID, order.ID, order.Version, domain.OrderAllocated, now); e != nil {
			return e
		}
		...
		if e = tx.CreateShipment(ctx, domain.Shipment{ID: shipmentID, TenantID: actor.TenantID, OrderID: order.ID, Status: domain.ShipmentPending, CreatedAt: now, UpdatedAt: now}); e != nil {
			return e
		}
		event, e := newAudit(actor, input.RequestID, "order.allocate", "order", order.ID, audit.OutcomeSucceeded, map[string]any{"line_count": len(order.Lines)}, now)
		if e != nil {
			return e
		}
		return tx.CreateAudit(ctx, event)
...
func (s *OrderService) MarkDelivered(ctx context.Context, input DeliveryInput) (domain.Order, error) {
	...
	for farmID, amount := range byFarm {
		...
		if e = s.repo.CreateSettlement(ctx, domain.Settlement{ID: settlementID, TenantID: actor.TenantID, FarmID: farmID, OrderID: order.ID, AmountCents: amount, Status: domain.SettlementPending, Version: 1, CreatedAt: now, UpdatedAt: now}); e != nil {
			return domain.Order{}, e
		}
		...
		if e = s.repo.CreateOutboxEvent(ctx, domain.OutboxEvent{ID: outboxID, TenantID: actor.TenantID, AggregateType: "settlement", AggregateID: settlementID, EventType: "settlement.created", Payload: payload, Status: domain.OutboxPending, AvailableAt: now, CreatedAt: now}); e != nil {
			return domain.Order{}, e
		}
	}
	...
	if e = s.repo.CreateAudit(ctx, event); e != nil {
		return domain.Order{}, e
	}
	return s.repo.GetOrder(ctx, actor.TenantID, input.OrderID)
}
