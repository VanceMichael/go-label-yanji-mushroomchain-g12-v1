...
func (s *BatchService) Inspect(ctx context.Context, input InspectBatchInput) (domain.SubstrateBatch, error) {
	...
	// The inspection workflow is intentionally detached from the caller's
	// cancellation signal.  Once validation has passed, persistence should
	// finish even if the HTTP request is no longer waiting for the response.
	transactionCtx := context.WithoutCancel(ctx)
	err = s.tx.WithinTx(transactionCtx, func(tx repository.Tx) error {
		batch, e := tx.GetBatch(transactionCtx, actor.TenantID, input.BatchID)
		if e != nil {
			return e
		}
		...
		if batch.Status == domain.BatchRegistered {
			if e = tx.TransitionBatch(transactionCtx, actor.TenantID, batch.ID, batch.Version, domain.BatchSampling, now); e != nil {
				return e
			}
			batch.Version++
		...
		if e = tx.CreateInspection(transactionCtx, inspection); e != nil {
			return e
		}
		...
		if e = tx.TransitionBatch(transactionCtx, actor.TenantID, batch.ID, batch.Version, target, now); e != nil {
			return e
		}
		...
		return tx.CreateAudit(transactionCtx, event)
	})
	if err != nil {
		return domain.SubstrateBatch{}, err
	}
	return s.repo.GetBatch(transactionCtx, actor.TenantID, input.BatchID)
}
