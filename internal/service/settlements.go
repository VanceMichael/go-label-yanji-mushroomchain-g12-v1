...
func (s *SettlementService) MarkPaid(ctx context.Context, input PaySettlementInput) (domain.Settlement, error) {
	...
	if err = item.RequirePaymentVersion(input.ExpectedVersion); err != nil {
		return domain.Settlement{}, err
	}
	...
	if err = s.repo.CreateOutboxEvent(ctx, domain.OutboxEvent{ID: eventID, TenantID: actor.TenantID, AggregateType: "settlement", AggregateID: item.ID, EventType: "settlement.paid", Payload: payload, Status: domain.OutboxPending, AvailableAt: now, CreatedAt: now}); err != nil {
		return domain.Settlement{}, err
	}
	err = s.tx.WithinTx(ctx, func(tx repository.Tx) error {
